      ...
      $this->substitutions[EnumSubstitutionNamespace::MEMBER->value][EnumMemberSubstitutionKey::PROJECT_DATA->value] =  function(array $keyArg, ?Entities\Musician $musician) {
        ...
        $fieldsByType = [
          ...
          'deposit' => $participantFields->filter(function($field) {
            /** @var Entities\ProjectParticipantField $field */
            if (empty($field->getDepositDueDate())) {
              return false;
            }
            switch ($field->getDataType()) {
              case FieldDataType::RECEIVABLES:
  ...
  private function getHeaders(string $url, ?bool $sslVerify = null): ?int
  {
    ...
    $options = [
      ClientRequestOptions::HEADERS => [
        'User-Agent' => 'Mozilla/5.0  (X11; Linux x86_64; rv:150.0)',//  Gecko/20100101 Firefox/150.0', // 'Orgacloud/1.0',
    ...
    if ($response->getStatusCode() != Http::STATUS_OK) {
      $this->logInfo('STATUS CODE: "' . $response->getStatusCode() . '" REQUEST HEADERS ' . $url . ' ' . print_r($response->getHeaders(), true));
    }
  ...
  private function validateMessageHtml(string &$message, bool $repair = false): bool
  {
    ...
    foreach ($links as $item) {
      ...
      $linkStatus[] = [
        ...
        'text' => $text,
        'status' => $thisLinkGood,
        'explanations' => $thisLinkGood ?: $explanations ?? $this->l->t('The link could not be opened.'),
      ];
