...
class DatabaseStorageFolder extends DatabaseStorageDirEntry
{
  ...
  public function addSubFolder(string $name):DatabaseStorageFolder
  {
    ...
    if ($existing->count() > 1) {
      throw new Exceptions\DatabaseEntityExistsException(
        'Directory entry "' . $name . '" already exists multiple times in directory ' . $this->id,
        entityName: DatabaseStorageFolder::class,
      );
    }
    if ($existing->count() == 1) {
      ...
      if (!($dirEntry instanceof DatabaseStorageFolder)) {
        throw new Exceptions\DatabaseEntityExistsException(
          'Directory entry "' . $name . '" already exists in directory ' . $this->id . ' and the existing entry is not a directory.',
          $dirEntry,
        );
      }
      return $dirEntry;
  ...
  public function addDocument(
    ...
    if ($existing->count() > 1) {
      throw new Exceptions\DatabaseEntityExistsException(
        'Directory entry "' . $fileName . '" already exists multiple times in directory ' . $this->id,
        newEntity: $file,
      );
    }
    if ($existing->count() == 1) {
      ...
      switch ($conflictAction) {
        case EnumAddDocumentConflictAction::RENAME:
          $pathInfo = pathinfo($fileName);
          $namePart = $pathInfo['filename'] . '~';
          $extension = '.' . $pathInfo['extension'];
          $backups = $this->directoryEntries
            ->map(fn(DatabaseStorageDirEntry $value) => $value->getName())
            ->filter(fn(string $name) => str_starts_with($name, $namePart) && str_ends_with($name, $extension))
            ->toArray();
          $count = count($backups);
          do {
            ++$count;
            $backupName = $namePart . $count . $extension;
          } while (in_array($backupName, $backups));
          $dirEntry
            ->setName($backupName)
            ->setUpdated(new DateTimeImmutable);
          $dirEntry = null;
          // We now need to do a flush() to the database as there is a unique
          // constraint on the dir-entry names. Might be an anti-pattern to
          // put this into the entity ...
          EntityManager::getInstance()->flush();
          break;
        case EnumAddDocumentConflictAction::REPLACE:
          if (!($dirEntry instanceof DatabaseStorageFile)) {
            throw new Exceptions\DatabaseEntityExistsException(
              'Directory entry "' . $fileName . '" already exists in directory ' . $this->id . ' but the existing entry is not a file.',
              existingEntity: $dirEntry,
              newEntity: $file,
            );
          }
          $dirEntry
            ->setFile($file)
            ->setUpdated($dirEntry->getUpdated());
          break;
        case EnumAddDocumentConflictAction::FAIL:
          if (!($dirEntry instanceof DatabaseStorageFile)) {
            throw new Exceptions\DatabaseEntityExistsException(
              'Directory entry "' . $fileName . '" already exists in directory ' . $this->id . ' but the existing entry is not a file.',
              existingEntity: $dirEntry,
          ...
          if ($dirEntry->getFile() !== $file) {
            throw new Exceptions\DatabaseEntityExistsException(
              'Directory entry "' . $fileName . '" already exists in directory ' . $this->id . ' and the existing entry does not point to the same file.',
              existingEntity: $dirEntry,
              newEntity: $file,
            );
          }
