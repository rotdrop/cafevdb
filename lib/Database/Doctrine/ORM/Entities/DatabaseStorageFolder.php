...
class DatabaseStorageFolder extends DatabaseStorageDirEntry
{
   ...
   *
   * @param string $conflict Condflict resolution, fail, rename, replace.
   *
   * @return null|DatabaseStorageDirEntry The new or existing
   */
  public function addDocument(EncryptedFile $file, ?string $fileName = null, string $conflict = self::ADD_DOCUMENT_CONFLICT_FAIL):?DatabaseStorageFile
  {
    ...
    if ($existing->count() == 1) {
      ...
      $dirEntry = $existing->first();
      switch ($conflict) {
        case self::ADD_DOCUMENT_CONFLICT_RENAME:
          ...
          break;
        case self::ADD_DOCUMENT_CONFLICT_REPLACE:
          ...
          break;
        default:
        case self::ADD_DOCUMENT_CONFLICT_FAIL:
          if (!($dirEntry instanceof DatabaseStorageFile)) {
            throw new Exceptions\DatabaseException('Directory entry "' . $fileName . '" already exists in directory ' . $this->id . ' but the existing entry is not a file.');
          }
