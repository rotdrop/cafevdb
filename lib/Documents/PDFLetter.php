...
class PDFLetter extends \TCPDF
{
  ...
  {
    $addr1 = $this->getConfigValue('streetAddressName01');
    $addr2 = $this->getConfigValue('streetAddressName02');
    $street = $this->getConfigValue('streetAddressStreet');
    $street .= ' '.$this->getConfigValue('streetAddressHouseNumber');
    $ZIP = $this->getConfigValue('streetAddressZIP');
  ...
  public function testLetter($name = 'test.pdf', $dest = 'S')
  {
    // create a PDF object
    $pdf = new PDFLetter($this->configService, PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
...
70569 Stuttgart'
    ...
    $pdf->letter(
      $this->l10n->t('Salutation formula'),
      $this->l10n->t('Message Text'),
      $this->l10n->t('Closing Formula'),
      $this->l10n->t('Signature')
    );
