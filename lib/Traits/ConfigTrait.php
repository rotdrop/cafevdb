...
trait ConfigTrait
{
  ...
  {
    return (int)$this->getConfigValue(ConfigConstants::CLUB_MEMBER_PROJECT_ID_KEY, 0);
  }

  /** @return string */
  protected function getClubMembersProjectName():string
  {
    return $this->getConfigValue(ConfigConstants::CLUB_MEMBERS_PROJECT_KEY, '');
  }

  /** @return int */
  protected function getExecutiveBoardProjectId():int
  {
    return (int)$this->getConfigValue(ConfigConstants::EXECUTIVE_BOARD_PROJECT_ID_KEY, 0);
  }

  /** @return string */
  protected function getExcutiveBoardProjectName():string
  {
    return $this->getConfigValue(ConfigConstants::EXECUTIVE_BOARD_PROJECT_KEY, '');
  }

  /** @return null|string */
  protected function getSharedFolderPath():?string
  {
    return $this->getConfigValue(ConfigConstants::SHARED_FOLDER, null);
  }
  ...
  protected function getDocumentTemplatesPath(?string $templateName = null, bool $dirName = false)
  {
    ...
    if (!empty($templateName)) {
      $subFolder = ConfigConstants::DOCUMENT_TEMPLATES[$templateName]['folder']??null;
      if (!empty($subFolder)) {
  ...
  {
    return $this->getSharedSubFolderPath(ConfigConstants::PROJECTS_FOLDER);
  }
  ...
  {
    return $this->getSharedSubFolderPath(ConfigConstants::FINANCE_FOLDER);
  }
  ...
  protected function getBankTransactionsPath():?string
  {
    ...
    $transactionsFolder = $this->getConfigValue(ConfigConstants::TRANSACTIONS_FOLDER);
  ...
  {
    return $this->getSharedSubFolderPath(ConfigConstants::POSTBOX_FOLDER);
  }
  ...
  {
    return $this->getSharedSubFolderPath(ConfigConstants::OUTBOX_FOLDER);
  }
  ...
  protected function toolTipsService():ToolTipsService
  {
    if (empty($this->toolTipsService)) {
      ...
      if (!empty($this->toolTipsService)) {
        $this->toolTipsService->debug($this->shouldDebug(ConfigConstants::DEBUG_TOOLTIPS));
      }
    }
  ...
  {
    return array_reduce(
      ConfigConstants::APP_DB_KEYS,
      fn(bool $carry, string $key) => $carry && !empty($this->getConfigValue($key, null)),
      true,
    );
  }
  ...
  {
    $debugMode = (int)$this->getUserValue(EnumPersonalSettingsKey::DEBUG_MODE, 0);
