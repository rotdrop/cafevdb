  ...
  protected function configure()
  {
    $this
      ...
      ->addOption(
        ...
        'Restrict the operation to the given calendar, specified by its internal URI ' . implode(', ', array_keys(ConfigConstants::CALENDARS)) . '. CALENDAR may also consist of a comma separated list of several calendar-uris. If not given the act on all calendars.',
      )
      ->addOption(
        'all',
        'a',
  ...
  protected function execute(InputInterface $input, OutputInterface $output): int
  {
    ...
    if (!empty($calendars)) {
      ...
      foreach ($calendars as $calendar) {
        if (!in_array($calendar, array_keys(ConfigConstants::CALENDARS))) {
          $output->writeln(
            '<error>'
            . $this->l->t('CALENDAR "%1$s" must be one of "%2$s"', [
              $calendar,
              implode('", "', array_keys(ConfigConstants::CALENDARS)),
            ])
            . '</error>'
          );
          $output->writeln('');
      ...
      $calendars = array_keys(ConfigConstants::CALENDARS);
    }
