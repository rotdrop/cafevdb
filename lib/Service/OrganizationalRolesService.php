...
class OrganizationalRolesService
{
  ...
  private ?Entities\Project $executiveBoardProjectEntity = null;
  ...
  public function executiveBoardProject(): ?Entities\Project
  {
    ...
    $this->executiveBoardProjectEntity = $this->getDatabaseRepository(Entities\Project::class)->find($this->getExecutiveBoardProjectId());
    return $this->executiveBoardProjectEntity;
  }
  ...
  public function dedicatedBoardMemberParticipant(string $role, int $musicianId = 0):?Entities\ProjectParticipant
  {
    ...
    $participants = $this->executiveBoardMembers();
    return $participants->containsKey($musicianId) ? $participants->get($musicianId) : null;
  }
  ...
  /**
   * Check whether the given or currently logged in user is a member of the
   * executive board.
   *
   * @param string $uid
   *
  ...
  /**
   * Get the database entity for the matching executive board member.
   *
   * @param string $uid
   *
   * @return null|Entities\ProjectParticipant
  ...
  public function isClubMember(string $userId):bool
  {
    ...
    $clubMembersGid = CloudUserConnectorService::getProjectGroupId($clubMembersProjectId);
