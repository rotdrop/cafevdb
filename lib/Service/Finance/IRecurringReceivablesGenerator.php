...
interface IRecurringReceivablesGenerator
{
   ...
   * @var int
   *
   * The balancing account may be edited in the per-musician view.
   */
  const UI_EDITABLE_BALANCING_ACCOUNT = (1 << 5);

  /**
   * @var int
   *
   * The balancing account should be protected in the per-musician view.
   */
  const UI_PROTECTED_BALANCING_ACCOUNT = (1 << 6);

  /**
   * @var int
   *
   * The balancing account is visible in the per-musician view.
   */
  const UI_VISIBLE_BALANCING_ACCOUNT = (1 << 7);
  ...
  const UI_FLAGS = [
    ...
    self::UI_PROTECTED_BALANCING_ACCOUNT,
  ...
  /**
   * Sub-account name (leaf) where for the bookkeeping of receivables
