...
$instruments = CAFEVDB_mySQL::multiKeys('Musiker', 'Instrumente', $this->dbh);
...
if (!$this->myquery($sqlquery)) {
  CAFEVDB_Util::error("Could not execute the query\n".$sqlquery."\nSQL-Error: ".mysql_error(), true);
}

// Now do the same with the Besetzungen-table
$instruments = CAFEVDB_mySQL::multiKeys('Besetzungen', 'Instrument', $this->dbh);
array_push($instruments, $newvals['Instrument']);
sort($instruments, SORT_FLAG_CASE|SORT_STRING);
...
$instruments = CAFEVDB_mySQL::multiKeys('Projekte', 'Besetzung', $this->dbh);
array_push($instruments, $newvals['Instrument']);
sort($instruments, SORT_FLAG_CASE|SORT_STRING);
