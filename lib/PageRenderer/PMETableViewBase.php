...
abstract class PMETableViewBase extends AbstractPageRenderer
{
  ...
  protected function __construct(
    ...
    $this->showDisabled = $this->getUserValue(EnumPersonalSettingsKey::SHOW_DISABLED, false) === 'on';
    $this->expertMode = $this->getUserValue(EnumPersonalSettingsKey::EXPERT_MODE, false) === 'on';
    $this->financeMode = $this->getUserValue(EnumPersonalSettingsKey::FINANCE_MODE, false) === 'on';
    $this->filterVisibility = $this->getUserValue(EnumPersonalSettingsKey::INITIAL_FILTER_VISIBILITY, 'off') == 'on';

    $this->debugRequests = 0 != ((int)$this->getUserValue(EnumPersonalSettingsKey::DEBUG_MODE, 0) & ConfigConstants::DEBUG_REQUEST);
    ...
    $cgiDefault = [
      ...
      'recordsPerPage' => fn($value):int => (int)($value ?? $this->getUserValue(EnumPersonalSettingsKey::PAGE_ROWS_DEFAULT, 20)),
    ];
