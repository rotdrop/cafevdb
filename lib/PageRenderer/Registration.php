...
class Registration
{
  private const LEGACY_TEMPLATES = [
    ...
    Invoices::TEMPLATE => Invoices::class,
  ...
  public static function register(IRegistrationContext $context):void
  {
    ...
    $context->registerService(DataConstants::RENDERER_PREFIX_TAG . 'maintenance/debug', function($c) {
      return new class extends Renderer {}; // do nothing
    });

    foreach (self::LEGACY_TEMPLATES as $template => $class) {
      $context->registerServiceAlias(DataConstants::RENDERER_PREFIX_TAG . $template, $class);
      ...
      if (str_contains($template, Constants::PATH_SEP)) {
        $context->registerServiceAlias(DataConstants::RENDERER_PREFIX_TAG . str_replace(Constants::PATH_SEP, ':', $template), $class);
      }
    }
