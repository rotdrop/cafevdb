...
trait ParticipantFieldsTrait
{
   ...
   * field-type FieldDataType::RECEIVABLES,
   * FieldDataType::LIABILITIES, it is indexed by field-name. Steps to perform:
   *
  ...
  public function renderParticipantFields(
    ...
    $generator = function(array &$fieldDescData, array &$subTotals = []) use ($participantFields, $financeTab) {
      ...
      foreach ($participantFields as $field) {
        ...
        $subTotalsSign = $fieldDataType == FieldDataType::LIABILITIES ? '-1 * ' : '';

        if (!$this->participantFieldsService->isSupportedType($fieldMultiplicity, $fieldDataType)) {
          throw new Exception(
            $this->l->t(
              'Unsupported multiplicity / data-type combination: %s / %s',
              [ $fieldMultiplicity, $fieldDataType ]));
        }
        ...
        foreach ([ &$keyFdd, &$valueFdd ] as &$fdd) {
          switch ($fieldDataType) {
            ...
            case FieldDataType::LIABILITIES:
              $style = $this->defaultFDD[$fieldDataType->value];
              if (empty($style)) {
                throw new Exception($this->l->t('Not default style for "%s" available.', $fieldDataType));
              }
              unset($style['name']);
...
/>';
        switch ($fieldMultiplicity) {
          case FieldMultiplicity::SIMPLE:
            ...
            $valueFdd['css']['postfix'][] = $fieldDataType->value;
            $valueFdd['css']['postfix'][] = cat('data-type-', $fieldDataType);
  ...
  AS ' . self::MONETARY_TYPE . '
   ...
   href="'.$downloadLink.'">' . $html . '</a>';
                ...
                // FieldMultiplicity::SIMPLE and FieldDataType::RECEIVABLES or
                // FieldDataType::LIABILITIES and IFF the deposit-due-date field
  ...
  ' . $linkText . '
          ...
          case FieldMultiplicity::SINGLE:
            ...
            $keyFdd['css']['postfix'][] = $fieldDataType->value;
              ...
              case FieldDataType::DATETIME:
                if (!empty($dataValue)) {
                  try {
                    ...
                    $dataValue = ($fieldDataType == FieldDataType::DATE)
  ...
  ' . $linkText . '
                ...
                $keyFdd['select'] = 'M';
                $keyFdd['css']['postfix'][] = cat($fieldDataType->value);
                $keyFdd['css']['postfix'][] = cat('data-type-', $fieldDataType);
                break;
              case FieldDataType::DB_FILE:
                $this->joinStructure[$tableName]['flags'] |= self::JOIN_READONLY;
   ...
   href="'.$downloadLink.'">' . $fileBase . '.' . $extension . '</a>';
              ...
              case FieldDataType::LIABILITIES:
                foreach ($dataOptions as $dataOption) {
                  ...
                  $values2[$key] = $this->allowedOptionLabel($label, $data, $fieldDataType);
                }
                unset($keyFdd['mask']);
                $keyFdd['escape'] = false;
                ...
                if ($fieldDataType == FieldDataType::RECEIVABLES || $fieldDataType == FieldDataType::LIABILITIES) {
                  ...
                  // yet another field to support summing up totals
                  if ($fieldMultiplicity == FieldMultiplicity::MULTIPLE) {
                    $optionValueSql = 'IF(
  ...
  NULL
          ...
          case FieldMultiplicity::RECURRING:
            ...
            foreach ([&$keyFdd, &$valueFdd] as &$fdd) {
              $fdd['css']['postfix'][] = $fieldMultiplicity->value;
              $fdd['css']['postfix'][] = 'generated';
              $fdd['css']['postfix'][] = $fieldDataType->value;
  ...
  NULL
              ...
              // in contrast to FieldMultiplicity::PARALLEL and ::MULTIPLE the
  ...
  ORDER BY $order_by)
   ...
   href="'.$downloadLink.'">' . $fileBase . '.' . $extension . '</a>';
                ...
                default:
                  ...
                  foreach ($options as $optionKey => $fieldOption) {
                    ...
                    $rowHtml = $this->allowedOptionLabel($label, $optionValue, $fieldDataType);
   ...
   href="'.$downloadLink.'">' . $rowHtml . '</a>';
            ...
            $keyFdd['php|ACP'] = function($value, $op, $k, $row, $recordId, $pme) use ($field, $fieldDataType) {
              return '';
            };
            $keyFdd['input|ACP'] = 'HR';
            $valueFdd['php|ACP'] = function(
              ...
              $fieldId = $field->getId();
              $fieldMultiplicity = FieldMultiplicity::RECURRING;
  ...
  <tbody>';
              ...
              if (is_callable($recomputeAllLabel)) {
                $recomputeAllLabel = $recomputeAllLabel($fieldDataType);
              }
          ...
          case FieldMultiplicity::GROUPOFPEOPLE:
            ...
            $css[] = FieldMultiplicity::GROUPOFPEOPLE->value;
            $css[] = 'single-valued';
...
WHERE pp.project_id = $this->projectId",
  ...
  "limit", '.$max.'
            ...
            if ($fieldDataType == FieldDataType::RECEIVABLES || $fieldDataType == FieldDataType::LIABILITIES) {
              ...
              $groupMemberFdd['css']['postfix'][] = cat($fieldDataType);
              ...
              $groupMemberFdd['display|ACP'] = array_merge(
                ...
                [
                  ...
                  'postfix' => function($op, $pos, $k, $row, $pme) use ($fieldData, $fieldDataType, $tableName) {
                    $selectedKey = $row[$this->joinQueryField($tableName, 'option_key')];
                    $active = ($op == 'display' && empty($selectedKey)) ? '' : 'selected';
                    return $this->allowedOptionLabel('', $fieldData, $fieldDataType, $active)
                      .'</label>';
                  },
                ]);
...
WHERE pp.project_id = $this->projectId",
  ...
  "limit", $table.group_limit
                ...
                'display|ACP' => [
                  ...
                  'postfix' => function($op, $pos, $k, $row, $pme) use ($dataOptions, $fieldDataType, $tableName) {
                    $selectedKey = $row[$this->joinQueryField($tableName, 'option_key')];
                    $html = '';
                    foreach ($dataOptions as $dataOption) {
                      $key = $dataOption['key'];
                      ...
                      $html .= $this->allowedOptionLabel(
                        $dataOption['label'], $dataOption['data'], $fieldDataType, $active, [ 'key' => $dataOption['key'], ]);
                    }
                    $html .= '</label>';
                    return $html;
                  },
...
GROUP BY " . $fdAlias . ".option_key",
   ...
   *
   * @param FieldDataType $fieldDataType String-value of the FieldDataType.
   *
   ...
   */
  protected function allowedOptionLabel(
    string $label,
    mixed $value,
    ...
    $innerCss = $fieldDataType->value;
    $css = $innerCss . (empty($css) ? '' : ' ' . $css);
    $htmlData = [];
      ...
      case FieldDataType::DATETIME:
        if (!empty($value)) {
          try {
            ...
            $value = ($fieldDataType == FieldDataType::DATE)
              ? $this->dateTimeFormatter()->formatDate($date, 'medium')
              : $this->dateTimeFormatter()->formatDateTime($date, 'medium', 'short');
          } catch (Throwable $t) {
            // don't care
  ...
  /**
   * Generate one row for FieldMultiplicity::RECURRING in Change/Add/Paste mode.
   *
      ...
      switch ($fieldDataType) {
        ...
        case FieldDataType::DATETIME:
          try {
            ...
            $optionValue = ($fieldDataType == FieldDataType::DATE)
              ? $this->dateTimeFormatter()->formatDate($date, 'medium')
              : $this->dateTimeFormatter()->formatDateTime($date, 'medium', 'short');
          } catch (Throwable $t) {
            // ignore for now
  ...
  public function beforeUpdateSanitizeParticipantFields(PHPMyEdit &$pme, string $op, string $step, array &$oldValues, array &$changed, array &$newValues):bool
  {
    ...
    foreach ($this->project['participantFields'] as $participantField) {
      ...
      $fieldMultiplicity = $participantField['multiplicity'];
      $fieldDataType = $participantField['dataType'];
      ...
      switch ($fieldMultiplicity) {
        ...
        case FieldMultiplicity::GROUPSOFPEOPLE:
          ...
          if ($fieldMultiplicity == FieldMultiplicity::GROUPOFPEOPLE) {
            ...
            // $this->logInfo('OPTION: ' . $newGroupId . ' ' . dump($newDataOption));
          }
          ...
          if ($fieldMultiplicity == FieldMultiplicity::GROUPOFPEOPLE && !empty($newMembers)) {
  ...
  private static function fetchValueOptions(Entities\ProjectParticipantField $field, array $values, ?bool &$labelled = null)
  {
    ...
    uasort($options, function($a, $b) use ($values) {
      ...
      $cmp = strCmpEmptyLast($a->getLabel(), $b->getLabel());
      if ($cmp === 0) {
        $cmp = strCmpEmptyLast($a->getData(), $b->getData());
      }
      if ($cmp === 0) {
        $cmp = strCmpEmptyLast($values[(string)$a->getKey()], $values[(string)$b->getKey()]);
      }
      return $cmp;
    });
    return $options;
