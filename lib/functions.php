  ...
  class Util
  {
    ...
    static public function quasiCSVSplit($values, $delimiters = ':,;')
    {
      ...
      if (!isset($matches[2])) {
        ...
        $matches = preg_replace('/[\\\\]([^\\\\])/', '${1}', $matches[2]);
      }
     ...
     * $delimiters is used as CSV separator. Fields which contain
     * quotes (single or double) or delimiters are enclosed in quotes,
     * with characters escaped by a backslash as needed. Backslashes
     * are converted to double back-slashes.
     *
     */
    static public function quasiCSVJoin($values, $delimiters = ':,;')
    {
      foreach ($values as &$value) {
        ...
        if ($doubleQuotes > 0 && $doubleQuotes > $singleQuotes) {
          // escape single quotes, enclose in single quotes
          $value = str_replace("'", "\\'", $value);
          $value = "'".$value."'";
        } else if ($singleQuotes > 0) {
          // escape double quotes, enclose in double quotes
          $value = str_replace('"', '\\"', $value);
          $value = '"'.$value.'"';
        } else if ($limiters > 0) {
