...
class MusiciansRepository extends EntityRepository
{
   ...
   *
   * @param null|string $indexBy If non null index by the given column.
   *
   * @return null|array
   */
  public function fetchLastModifiedDate(array $criteria = [], ?string $indexBy = null):?array
  {
    $selects = [];
    $selects[]  = "GREATEST(
  ...
  AS lastModified";
    ...
    if ($indexBy !== null) {
      ...
      $qb = $this->createQueryBuilder('m', 'm.uuid');
    }
    $qb->select($selects)
  ...
  public function findByInstruments(
    ...
    return $this->findBy(
      [ 'instruments.instrument' => $instrumentIds ],
      [ 'id' => 'INDEX' ]);
  }
