...
class NumberFormatter
{
   ...
   */
  public function currencyToWords(mixed $value, ?string $currencyISOCode = null):string
  {
    if ($value instanceof RationalNumber) {
      $value = $value->mul(100)->round(0)->getWholePart();
    } else {
   ...
   * @return string Textual representation of the given currency value.
   */
  public function numberToWords(mixed $value, int $minDigits = 1, int $maxDigits = 2):string
  {
    return $this->numberToWordsPhp(self::ensureFloat($value, -1), $minDigits, $maxDigits);
  }
   ...
   * @return string Textual representation of the given currency value.
   */
  protected function numberToWordsKWN(float $value, int $minDigits = 1, int $maxDigits = 2):string
  {
   ...
   * @return string Textual representation of the given currency value.
   */
  protected function numberToWordsPhp(float $value, int $minDigits = 1, int $maxDigits = 2):string
  {
    $minDigits = min($minDigits, $maxDigits);
    $maxDigits = max($minDigits, $maxDigits);
    $value = round($value, $maxDigits);
   ...
   * @return float
   */
  private static function ensureFloat(mixed $value, int $scale = 2):float
  {
    if ($value instanceof RationalNumber) {
      $value = $value->toDecimal($scale);
