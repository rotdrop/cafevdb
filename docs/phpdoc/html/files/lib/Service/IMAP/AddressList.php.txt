...
use OCA\CAFEVDB\Wrapped\Horde_Mail_Rfc822_Object;
...
class AddressList implements Countable, JsonSerializable {
  ...
  public static function fromHorde(Horde_Mail_Rfc822_List $hordeList) {
    ...
    \OCP\Server::get(\Psr\Log\LoggerInterface::class)->info('#ADDRESSES ' . count($addresses));
    if (count($addresses) > 0) {
      \OCP\Server::get(\Psr\Log\LoggerInterface::class)->info('FIRST EMAIL ' . $addresses[0]->getEmail());
    }
