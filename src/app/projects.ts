...
const instrumentationNumbersPopup = (containerSel: string, post: AsyncNextcloudEvents[typeof BusEvents.PROJECT_INSTRUMENTATION_NUMBERS_POPUP]) => {
  ...
  return PHPMyEdit.tableDialogOpen(tableOptions, { Transpose: 'transposed', InhibitTranspose: 'true' });
};
...
const pmeFormInit = (containerSel: string|JQuery) => {
  ...
  if ($form.find(submitSel).length > 0) {
    ...
    const $name = $container.find<HTMLInputElement>(nameSelector);
    const $year = $container.find<HTMLSelectElement>(yearSelector);
    ...
    $registrationStart.on('blur, change', function() {
      const $this = $(this);
  ...
  $downloadShareContainer
    ...
    .on('blur', 'input.share-expiration-date', function() {
      const $this = $(this);
      const projectId = $form.find('input[name="projectId"]').val();
      setBusyIndicators(true, $container, true);
  ...
  $downloadShareContainer.find('.operation.button.copy-to-clipboard')
    ...
    .on('click', function() {
      ...
      const url = $controlContainer.find<HTMLAnchorElement>('a.url.external').attr('href') ?? '';
  ...
  $form.find('.mailing-list-dropdown .list-action').on('click', function() {
    const $this = $(this);
...
type ProjectWebPageRequest = {
  action: EnumProjectWebPagesAction;
  articleId: number;
  projectId: number;
  articleData?: ProjectWebPage;
};
 ...
 */
const projectWebPageTabHandler = function(event: JQuery.Event, ui: JQueryUI.TabsActivationUIParams, $container: JQuery) {
  const tabId = ui.newTab.attr('id')!;
  // alert('id' + tabId);
  let projectId: number;
  let articleId: number;
  let articleData: ProjectWebPage;
  switch (tabId) {
...
const articleSelectOnChange = function(this: HTMLSelectElement, $container: JQuery) {
  const $this = $(this);
...
type ContainerContext = {
  $container: JQuery;
  $articleBox: JQuery;
  $allDisplayFrames: JQuery<HTMLIFrameElement>;
  numDisplayFrames: number;
  numChangeFrames: number;
  resizeCB: () => void;
};

const attachArticleSelectHandlers = function(containerContext: ContainerContext) {
  const $container = containerContext.$container;
  const $articleBox = containerContext.$articleBox;
  const $articleSelect = $container.find<HTMLSelectElement>('#cmsarticleselect');
  ...
  $articleSelect.on('change', function(this: HTMLSelectElement) {
    return articleSelectOnChange.call(this, $container);
  });
};

const scrollbarAdjust = function(containerContext: ContainerContext) {
 ...
 *
 * @param containerContext TBD.
 *
 * @param $iframe TBD.
 */
const forceHeight = function(containerContext: ContainerContext, $iframe: JQuery<HTMLIFrameElement>) {
...
const displayArticleLoad = function(containerContext: ContainerContext, iframe?: HTMLIFrameElement) {
...
const changeArticleLoad = function(containerContext: ContainerContext, iframe?: HTMLIFrameElement) {
  const $container = containerContext.$container;
...
const tableLoadCallback = function(
  ...
  if (parameters.reason === 'dialogClose') {
    if (parameters.closedBy !== undefined && parameters.closedBy === pmeSys('savedelete')) {
      const templateRendererValue = $(parameters.tableDialogOptions!.ambientContainerSelector).find('input[name="templateRenderer"]').val();
      if (templateRendererValue !== templateRenderer(template)) {
  ...
  const $displayFrames = $articleBox.find<HTMLIFrameElement>('iframe.cmsarticleframe.display, iframe.cmsarticleframe.add');
  const $changeFrames = $articleBox.find<HTMLIFrameElement>('iframe.cmsarticleframe.change');
  const $allDisplayFrames = $articleBox.find<HTMLIFrameElement>('.cmsarticleframe.display');
  const $allChangeFrames = $articleBox.find<HTMLIFrameElement>('.cmsarticleframe.change');
  ...
  // loader
  $container.on('click', 'li.nav > a.nav', function() {
  ...
  const updateInstrumentVoices = function(additionalVoices?: Record<string, number>) {
    ...
    const $instrumentationVoicesSelect = $container.find<HTMLSelectElement>('select.projects--instrumentation-voices');
  ...
  const inputVoicesHandler = function(input: HTMLInputElement|JQuery<HTMLInputElement>) {
    const $this = jq(input);
  ...
  $container.on('blur', 'div.instrument-voice.request.container input.instrument-voice.input', function() {
    return inputVoicesHandler(this);
  });

  $container.on('click', 'div.instrument-voice.request.container input.instrument-voice.confirm', function() {
    const $this = $(this);
    const instrument = $this.data('instrument');
    return inputVoicesHandler($this.parent().find('input.input.instrument-' + instrument));
  });

  $container.on('change', 'select.projects--instrumentation, select.projects--instrumentation-voices', function() {
    const $instrumentationVoicesSelect = $container.find<HTMLSelectElement>('select.projects--instrumentation-voices');

    // intercept request to enter voices-number manually
    const selectedVoices = SelectUtils.selected($instrumentationVoicesSelect) as string[];
