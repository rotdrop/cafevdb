...
function manageDialogToBackButtons() {
  ...
  const relevantDialogs = $('.ui-dialog.ui-widget')
    ...
    .elements().filter(([_index, element]) => parseInt(getComputedStyle(element)?.getPropertyValue('z-index')) > overlayIndex && $(element).is(':visible'));
 ...
 */
function dialogToBackButton($dialogHolder: JQuery) {
 ...
 * @param $dialogHolder TBD.
 *
 * @param onChangeCallback TBD.
 */
function dialogFullScreenButton(
  $dialogHolder: JQuery,
  onChangeCallback: (mode: 'fullscreen'|'dialog', when: 'before'|'after') => void,
) {
  const $dialogWidget = $dialogHolder.dialog('widget');
 ...
 * @param callback TBD.
 */
function dialogCustomCloseButton(
  $dialogHolder: JQuery,
  callback: (event: JQuery.ClickEvent, $dialogHolder: JQuery) => void,
) {
  const $dialogWidget = $dialogHolder.dialog('widget');
