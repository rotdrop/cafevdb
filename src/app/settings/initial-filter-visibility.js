...
import { SET_INITIAL_FILTER_VISIBILITY as EVENT } from '../../event-bus-events.js';
...
const setter = (value, showMessage, $control, callbacks) => {
  ...
  globalState.PHPMyEdit.initialFilterVisibility = value;
  if (value) {
    $('input.pme-search').trigger('click');
  } else {
    $('input.pme-hide').trigger('click');
  }
  $('.personal-settings input[type="checkbox"].filtervisibility').prop('checked', value);
  return new Promise((resolve, reject) =>
    $.post(setPersonalUrl('filtervisibility'), { value })
