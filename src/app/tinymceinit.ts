...
const replaceImageSource = (editor: Editor, src: string, id: string) => {
...
const isAbsoluteUrl = (url: string) => /^https?:\/\/[\w\-/+=.,!;:&%@^~(){}?#]+$/i.test(url);
...
const myConfig = {
  ...
  setup(editor: Editor) {
    ...
    editor.on('PastePostProcess', function(event) {
      $(event.node).find('img').each(function(this: HTMLImageElement) {
        const id = 'i' + Date.now();
        ...
        this.onload = function(this: HTMLImageElement) {
          ...
          console.error('ONLOAD', { self: this, args: arguments });
          this.onload = null;
  ...
  init_instance_callback(inst: Editor) {
    ...
    mceWindow.addEventListener('resize', () => {
    ...
    if ($mceElement.prop('disabled') || $mceElement.prop('readonly')) {
      ...
      inst.getBody().removeAttribute('contenteditable');
    }
  },
...
const mySmallConfig = {
  ...
  init_instance_callback(editor: Editor) {
    ...
    editor.on('FullscreenStateChanged', function(event) {
      if (!event.state) {
        $('input#focusstealer').trigger('focus').trigger('blur');
      } else {
        editor.focus(true);
      }
    });
...
const myGetConfig = function(plusConfig: Record<string, unknown>) {
