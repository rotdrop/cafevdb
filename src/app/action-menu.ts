...
import { appContainerSelector } from './globals.ts';
import $, { jq } from './jquery.ts';

/**
 * @param $dropDownContainer TBD.
 */
function markDialog($dropDownContainer: JQuery) {
...
const hideActionMenu = <T extends HTMLElement>($dropDownContainer: JQuery<T>) => {
  $dropDownContainer
...
const enforceVisibility = <T extends HTMLElement>($element: JQuery<T>) => {
...
const installActionMenuHandlers = <T extends HTMLElement>($container: JQuery<T>) => {
  $container = $container || $(appContainerSelector);

  $container.on('click', '.dropdown-container.dropdown-no-hover', function(this: HTMLElement, event) {
...
/**
 * @param element Element or selector. If omitted all action menus are
 * closed.
 */
function closeActionMenu<T extends HTMLElement>(element?: string|T|JQuery<T>) {
