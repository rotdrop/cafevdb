...
const toRgb = (s: string) => {
  ...
  function stepCalc(steps: number, ends: Color[]) {
    const step = <number[]>[];
  ...
  function mixPalette(steps: number, color1: Color, color2: Color) {
    var palette = <Color[]>[]
    palette.push(color1)
  ...
  function hashToInt(hash: string, maximum: number) {
    let finalInt = 0
    const result = <number[]>[]
...
$.fn.imageplaceholder = function(seed: string, text: string, size: number) {
  text = text || seed
